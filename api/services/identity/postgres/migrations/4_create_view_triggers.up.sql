...
DECLARE created_role_id admins.admin_id % TYPE;
...
SELECT *
FROM roles_with_permissions INTO created_role_with_permissions
WHERE role_id = created_role_id;
RETURN created_role_with_permissions;
END IF;
FOREACH input_permission_method IN ARRAY NEW.permissions LOOP
...
INSERT ON roles_with_permissions FOR EACH ROW EXECUTE PROCEDURE create_role_with_permissions();
-- INSTEAD OF UPDATE TRIGGER
CREATE OR REPLACE FUNCTION UPDATE_ROLE_WITH_PERMISSIONS() RETURNS TRIGGER AS $$
DECLARE old_permission_method permissions.method % TYPE;
...
SELECT *
FROM roles_with_permissions INTO updated_role_with_permissions
WHERE role_id = NEW.role_id;
RETURN updated_role_with_permissions;
END IF;
-- Grant new permissions
FOREACH new_permission_method IN ARRAY NEW.permissions LOOP IF NOT new_permission_method = ANY (OLD.permissions) THEN
INSERT INTO permissions(role_id, method)
VALUES (NEW.role_id, new_permission_method);
END IF;
END LOOP;
-- Deny permissions
FOREACH old_permission_method IN ARRAY OLD.permissions LOOP IF NOT old_permission_method = ANY (NEW.permissions) THEN
DELETE FROM permissions
WHERE role_id = NEW.role_id
	AND method = old_permission_method;
END IF;
END LOOP;
SELECT *
FROM roles_with_permissions INTO updated_role_with_permissions
...
UPDATE ON roles_with_permissions FOR EACH ROW EXECUTE PROCEDURE UPDATE_ROLE_WITH_PERMISSIONS();
/*
 * Admins related triggers
 */
-- INSTEAD OF INSERT TRIGGER
CREATE OR REPLACE FUNCTION create_admin_with_roles() RETURNS TRIGGER AS $$
DECLARE created_admin_id admins.admin_id % TYPE;
created_admin_with_roles admins_with_roles % ROWTYPE;
input_role_id roles.role_id % TYPE;
BEGIN ASSERT NEW.registered IS NULL,
'registered field is output only';
...
SELECT *
FROM admins_with_roles INTO created_admin_with_roles
WHERE admin_id = created_admin_id;
RETURN created_admin_with_roles;
END IF;
FOREACH input_role_id IN ARRAY NEW.roles LOOP
...
INSERT ON admins_with_roles FOR EACH ROW EXECUTE PROCEDURE create_admin_with_roles();
-- INSTEAD OF UPDATE TRIGGER
CREATE OR REPLACE FUNCTION update_admin_with_roles() RETURNS TRIGGER AS $$
DECLARE old_role_id roles.role_id % TYPE;
new_role_id roles.role_id % TYPE;
updated_admin_with_roles admins_with_roles % ROWTYPE;
BEGIN ASSERT NEW.registered = OLD.registered,
'registered field is output only';
UPDATE admins
...
SELECT *
FROM admins_with_roles INTO updated_admin_with_roles
WHERE admin_id = NEW.admin_id;
RETURN updated_admin_with_roles;
END IF;
-- Grant new roles 	
FOREACH new_role_id IN ARRAY NEW.roles LOOP IF NOT new_role_id = ANY (OLD.roles) THEN
INSERT INTO admins_roles(admin_id, role_id)
VALUES (NEW.admin_id, new_role_id);
END IF;
END LOOP;
-- Deny roles
FOREACH old_role_id IN ARRAY OLD.roles LOOP IF NOT old_role_id = ANY (NEW.roles) THEN
...
UPDATE ON admins_with_roles FOR EACH ROW EXECUTE PROCEDURE update_admin_with_roles();
/*
 * Users related triggers
 */
-- INSTEAD OF INSERT TRIGGER
CREATE OR REPLACE FUNCTION create_user_with_roles() RETURNS TRIGGER AS $$
DECLARE created_user_id users.user_id % TYPE;
...
IF NEW.roles IS NULL THEN
SELECT *
FROM users_with_roles INTO created_user_with_roles
...
INSERT ON users_with_roles FOR EACH ROW EXECUTE PROCEDURE create_user_with_roles();
-- INSTEAD OF UPDATE TRIGGER
...
IF NEW.roles = OLD.roles THEN
SELECT *
FROM users_with_roles INTO updated_user_with_roles
WHERE user_id = NEW.user_id;
...
FOREACH new_role_id IN ARRAY NEW.roles LOOP IF NOT new_role_id = ANY (OLD.roles) THEN
INSERT INTO users_roles(user_id, role_id)
VALUES (NEW.user_id, new_role_id);
END IF;
END LOOP;
-- Deny roles
FOREACH old_role_id IN ARRAY OLD.roles LOOP IF NOT old_role_id = ANY (NEW.roles) THEN
...
UPDATE ON users_with_roles FOR EACH ROW EXECUTE PROCEDURE update_user_with_roles();